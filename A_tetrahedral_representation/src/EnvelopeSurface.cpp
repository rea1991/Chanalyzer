#include "EnvelopeSurface.h"

void EnvelopeSurface::clear()
{
...
EnvelopeSurface::~EnvelopeSurface()
{
	clear();
}

void EnvelopeSurface::init()
{
...
EnvelopeSurface::EnvelopeSurface(DelPhiShared* ds):Surface()
{
	init();
	// set environment
	delphi = ds;	
...
EnvelopeSurface::EnvelopeSurface():Surface()
{
	delphi = NULL;
	init();
}

void EnvelopeSurface::init(ConfigFile* cf)
{
	double skin_s = cf->read<double>( "Envelope_Surface_Parameter", 0.45 );
...
EnvelopeSurface::EnvelopeSurface(ConfigFile* cf,DelPhiShared* ds):Surface(cf)
{
	init();
	init(cf);
	// set environment
...
void EnvelopeSurface::postRayCasting()
{
...
bool EnvelopeSurface::preBoundaryProjection()
{
...
bool EnvelopeSurface::build()
{
	bool f=false;
	#ifdef ENABLE_CGAL
		f = buildEnvelopeCGAL();
	#else
		cout << endl << ERR << "Envelope surface cannot be used, please install CGAL and rebuild\n";
		exit(-1);
	#endif
	if (!f)
	{
		cout << endl << ERR << "Error during Envelope build-up";
	}
	return f;
}

#ifdef ENABLE_CGAL
bool EnvelopeSurface::buildEnvelopeCGAL()
{
	...
	cout << endl << INFO << "Building Envelope surface..";
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////// Compute Voronoi points and reduced Delaunay solids (Envelope) ///////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for (;fcit!=rT.finite_cells_end();fcit++,currentCell++)    
	{
		...
		for (int i=0;i<4;i++)
		{
			...
			mc->weights[i] = wp.weight();

			a = vor[0]-mc->reduced[i][0];				
			double tempx = mc->reduced[i][0]+s*a;
							
			b = vor[1]-mc->reduced[i][1];
			double tempy = mc->reduced[i][1]+s*b;

			c = vor[2]-mc->reduced[i][2];
			double tempz = mc->reduced[i][2]+s*c;

			cout << endl << "point " << mc->points[i][0] << " " << mc->points[i][1] << " " << mc->points[i][2];
			cout << endl << "minkowsky " << tempx << " " << tempy << " " << tempz << endl;
		...
		cout << endl << "vor " << vor[0] << " " << vor[1] << " " << vor[2] << endl;
		tempMixedComplex.push_back(mc);

		// given the weights on the points and on the edges, compute the Weight function
		// for the tethraedron.
		// Starting with this function, we project the original points to their reduced form

		// compute weights on the edges
		//          v0
		//         /|\
		//        / | \
		//   e0  /  |  \ e3
		...
		double dd;
		DIST2(dd,mc->points[0],mc->points[1]);
		double w_e0 = .5*(mc->weights[0]+mc->weights[1])-s*dd*.25;

		DIST2(dd,mc->points[1],mc->points[2]);
		double w_e1 = .5*(mc->weights[1]+mc->weights[2])-s*dd*.25;

		DIST2(dd,mc->points[2],mc->points[3]);
		double w_e2 = .5*(mc->weights[2]+mc->weights[3])-s*dd*.25;

		DIST2(dd,mc->points[3],mc->points[0]);
		double w_e3 = .5*(mc->weights[3]+mc->weights[0])-s*dd*.25;

		DIST2(dd,mc->points[0],mc->points[2]);
		double w_e4 = .5*(mc->weights[0]+mc->weights[2])-s*dd*.25;

		DIST2(dd,mc->points[1],mc->points[3]);
		double w_e5 = .5*(mc->weights[1]+mc->weights[3])-s*dd*.25;

		double mid0[3],mid1[3],mid2[3],mid3[3],mid4[3],mid5[3];

		MID(mid0,mc->points[0],mc->points[1]);
		MID(mid1,mc->points[1],mc->points[2]);
		MID(mid2,mc->points[2],mc->points[3]);
		MID(mid3,mc->points[3],mc->points[0]);
		MID(mid4,mc->points[0],mc->points[2]);
		MID(mid5,mc->points[1],mc->points[3]);
		
		// planes for vertices
		// facet planes are planes_or
		double planeF0[4],planeF1[4],planeF2[4],planeF3[4];
		// edge planes are opposite to vertex i
		double planeE0[4],planeE1[4],planeE2[4],planeE3[4];

		// compute the 8 planes from which to deduce the total weight function on tethraedron
		// F0
		plane3points(mc->points[3],mc->points[1],mc->points[2],planeF0,true);		
		// F1
		plane3points(mc->points[3],mc->points[2],mc->points[0],planeF1,true);		
		// F2
		plane3points(mc->points[0],mc->points[1],mc->points[3],planeF2,true);
		// F4
		plane3points(mc->points[2],mc->points[1],mc->points[0],planeF3,true);
		// E0
		plane3points(mid0,mid3,mid4,planeE0,false);
		// E1
		plane3points(mid0,mid1,mid5,planeE1,false);
		// E2
		plane3points(mid1,mid2,mid4,planeE2,false);
		// E3
		plane3points(mid2,mid3,mid5,planeE3,false);

		// weight function for each cardinal point (vertices and edges)

		double Qv0[3][3],Qv1[3][3],Qv2[3][3],Qv3[3][3];
		double Qe0[3][3],Qe1[3][3],Qe2[3][3],Qe3[3][3],Qe4[3][3],Qe5[3][3];

		double av0[3],av1[3],av2[3],av3[3];
		double ae0[3],ae1[3],ae2[3],ae3[3],ae4[3],ae5[3];

		double cv0,cv1,cv2,cv3;
		double ce0,ce1,ce2,ce3,ce4,ce5;

		double n1,n2,normFactor;
		
		// W(v0)		
		planeByplane(planeE0,planeF0,Qv0,av0,cv0);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeE0,mc->points[0])+planeE0[3];
		n2 = DOT(planeF0,mc->points[0])+planeF0[3];
		normFactor = (mc->weights[0]/(n1*n2));
		...
		// W(v1)				
		planeByplane(planeE1,planeF1,Qv1,av1,cv1);

		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeE1,mc->points[1])+planeE1[3];
		n2 = DOT(planeF1,mc->points[1])+planeF1[3];
		normFactor = (mc->weights[1]/(n1*n2));
		...
		// W(v2)		
		planeByplane(planeE2,planeF2,Qv2,av2,cv2);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeE2,mc->points[2])+planeE2[3];
		n2 = DOT(planeF2,mc->points[2])+planeF2[3];
		normFactor = (mc->weights[2]/(n1*n2));
		...
		// W(v3)				
		planeByplane(planeE3,planeF3,Qv3,av3,cv3);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeE3,mc->points[3])+planeE3[3];
		n2 = DOT(planeF3,mc->points[3])+planeF3[3];
		normFactor = (mc->weights[3]/(n1*n2));
		...
		// W(e0)
		planeByplane(planeF0,planeF1,Qe0,ae0,ce0);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeF0,mid0)+planeF0[3];
		n2 = DOT(planeF1,mid0)+planeF1[3];
		normFactor = (w_e0/(n1*n2));
		...
		// W(e1)
		planeByplane(planeF1,planeF2,Qe1,ae1,ce1);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeF1,mid1)+planeF1[3];
		n2 = DOT(planeF2,mid1)+planeF2[3];
		normFactor = (w_e1/(n1*n2));
		...
		// W(e2)
		planeByplane(planeF3,planeF2,Qe2,ae2,ce2);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeF3,mid2)+planeF3[3];
		n2 = DOT(planeF2,mid2)+planeF2[3];
		normFactor = (w_e2/(n1*n2));
		...
		// W(e3)
		planeByplane(planeF3,planeF0,Qe3,ae3,ce3);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeF3,mid3)+planeF3[3];
		n2 = DOT(planeF0,mid3)+planeF0[3];
		normFactor = (w_e3/(n1*n2));
		...
		// W(e4)
		planeByplane(planeF0,planeF2,Qe4,ae4,ce4);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeF0,mid4)+planeF0[3];
		n2 = DOT(planeF2,mid4)+planeF2[3];
		normFactor = (w_e4/(n1*n2));
		...
		// W(e5)
		planeByplane(planeF3,planeF1,Qe5,ae5,ce5);
		// normalize quadratic form such that at point the value is equal to the weight
		n1 = DOT(planeF3,mid5)+planeF3[3];
		n2 = DOT(planeF1,mid5)+planeF1[3];
		normFactor = (w_e5/(n1*n2));
		...
		// combine results into one quadratic function
		// this the final weight function for the current tethraedron
		double Q[3][3],aa[3],cc;

		cout << endl;
		for (int i=0;i<3;i++)
		{
			aa[i]=av0[i]+av1[i]+av2[i]+av3[i]+ae0[i]+ae1[i]+ae2[i]+ae3[i]+ae4[i]+ae5[i];
			for (int j=0;j<3;j++)
			{
				Q[i][j]=Qv0[i][j]+Qv1[i][j]+Qv2[i][j]+Qv3[i][j]+Qe0[i][j]+Qe1[i][j]+Qe2[i][j]+Qe3[i][j]+Qe4[i][j]+Qe5[i][j];
				cout << Q[i][j] << " ";
			}
			cout << endl ;
		}
		cc = cv0+cv1+cv2+cv3+ce0+ce1+ce2+ce3+ce4+ce5;

		double ddd = -radius2*(1-s)+vor[0]*vor[0]+vor[1]*vor[1]+vor[2]*vor[2];		
		cout << endl << "ddd " <<ddd << " " << cc;

		cout << endl << "a " << aa[0] << " " << aa[1] << " " << aa[2]; 
		cout << endl << "a->vor " << -0.5*aa[0]/s << " " << -0.5*aa[1]/s << " " << -0.5*aa[2]/s; 

		exit(-1);
		// compute reduced mixed cell points
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////// Compute Voronoi (VDW) or reduced Voronoi solids (Envelope) //////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// Envelope Surface needs the remaining solids
	...
	{
		ofstream of;
		cout << endl << INFO << "Saving surface in Pov-Ray in envelope.pov...";
		cout.flush();
		of.open("envelope.pov");	
...
void EnvelopeSurface::saveEnvelopePatch(ofstream& of,MixedCell* mc,int index,vector<Weighted_point>& la)
{
...
void EnvelopeSurface::preProcessPanel()
{
...
bool EnvelopeSurface::buildAuxiliaryGrid()
{
	...
	// the macro is MAX_MIXEDCELLS in EnvelopeSurface.h.
...
/** save in .envelope format*/
bool EnvelopeSurface::save(char* fileName)
{
	ofstream fout;
    fout.open(fileName,ios::out);

	cout << endl << INFO << "Writing envelope in .envelope file format in " << fileName << "...";
...
bool EnvelopeSurface::load(char* fileName) 
{
	...
	//cout << endl << INFO << "Loading Envelope Surface in file " << fileName << "...";

	// TODO LOAD
	return true;
}

void EnvelopeSurface::printSummary()
{
	cout << endl << INFO << "Shrinking value " << getShrinking();
	if (mixedComplex == NULL)
	{
		cout << endl << WARN << "Envelope surface not loaded!";
	}
	else
	{
...
bool EnvelopeSurface::isFeasible(MixedCell* mc,double* point)
{	
...
inline bool EnvelopeSurface::rayQuadricIntersection(double* orig,double* dir,double*Q,double* i1,double* i2,int id,double* cache)
{
	double a=0,b=0,c=0;
...
inline void EnvelopeSurface::getRayIntersection(double pa[3],double pb[3],vector<pair<double,double*> >& intersections,int thdID,bool computeNormals)
{
	double dir[3];
	...
	// this cache exploits the particular structure of Envelope ray-tracing in order to improve speed.
...
bool EnvelopeSurface::getProjection(double p[3],double* proj1,double* proj2,
		double* proj3,double* normal1,double* normal2,double* normal3)
{
	...
	//	cout << endl << WARN << "Out of cube projection in EnvelopeSurface::getProjection!";
	
	return true;
}

void EnvelopeSurface::projectToQuadric(double* y,double* Q,int type,double* proj,double* normal,double& dist)
{	
