...
bool MeshSurface::loadPLY(char* fname) 
{
 ...
 {
	 cout << endl << ERR << "Unrecognized format " << formats << "\n";
	 return false;
 }
 ...
 for (i=0; i<nt; i++)
 {
  ...
  {   
   ...
   {
	   cout << endl << ERR << "\nloadPLY: Invalid index at face " << i << "\n";
	   return false;
   }
   for (j=3; j<=i4; j++)
   {
    if (i1 == i2 || i2 == i3 || i3 == i1) cout << endl << WARN << "\nloadPLY: Coincident indexes at triangle " << i << "! Skipping.\n";
    else
	{
		faceMatrix[i][0]=i1;
		faceMatrix[i][1]=i2;
    ...
    if (j<i4)
    {
	 ...
	 {
		 cout << endl << ERR << "\nloadPLY: Couldn't read indexes for face # " << i << "\n";
		 return false;
	 }
     else triangulate=1;
    }
