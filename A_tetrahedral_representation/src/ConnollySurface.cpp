...
//    Chanalyzer CAV = Chanalyzer();
//    CAV.compute_skeleton();
    
    
    //EXTRA RAFFO -- START
    
    //Compute regular triangulation:
    std::cout << std::endl << INFO << "Computing regular triangulation...";
    Rt reg_triang(l.begin(), l.end());
    
    //Compute alpha-shape:
    std::cout << std::endl << INFO << "Computing alpha shape complex...";
    Fixed_alpha_shape_3 alpha_shape(reg_triang, 0.0);
    
    if(find_geo_pockets==1)
    {
        auto Edelsbrunner_start = std::chrono::high_resolution_clock::now();
        
        Chanalyzer CAV = Chanalyzer();
        CAV.set_probeRadius(probe_radius);
        CAV.Edelsbrunner(reg_triang, alpha_shape);
        
        auto Edelsbrunner_stop = std::chrono::high_resolution_clock::now();
        auto Edelsbrunner_duration = std::chrono::duration_cast<std::chrono::seconds>(Edelsbrunner_stop - Edelsbrunner_start);
        std::cout << INFO << std::endl << "Time taken to detect pockets... "
        << Edelsbrunner_duration.count() << " seconds!" << std::endl;
    }
    else
        std::cout << INFO <<  "No detection method selected..." << std::endl;
	...
	for(;ffit!=alpha_shape.finite_facets_end();ffit++)
	{
		...
		if (ctype==Fixed_alpha_shape_3::REGULAR || ctype==Fixed_alpha_shape_3::SINGULAR)
		{
			...
			if( alpha_shape.classify(ffit->first) == Fixed_alpha_shape_3::INTERIOR)
			{
                ...
                const Point3& pp1 = Point3(p1.x(),p1.y(),p1.z());
                const Point3& pp2 = Point3(p2.x(),p2.y(),p2.z());
                const Point3& pp3 = Point3(p3.x(),p3.y(),p3.z());
                
                // get the proper normal
                const Vector3 nn = ( facetp.second % 2 == 1) ? CGAL::normal(pp1, pp2, pp3) : CGAL::normal(pp1, pp3, pp2);
