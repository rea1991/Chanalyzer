...
# PARAMETERS
diroff = dir.replace('D2_sections_analysis', 'B_channel_projection/output/frame.001/')
offname="channel"
dirtxt = dir.replace('D2_sections_analysis', 'D1_centerline_computation/output/frame.001/')
centerlinename="points"


def LoadSurface(filename,diroff):

    infile = open(diroff+"{}.off".format(filename), 'r')

    for line in itertools.islice(infile, 0, 3):
        pass
...
def LoadCenterLine(filename, dirtxt):

    infile = open(dirtxt+"{}.txt".format(filename), 'r')
...
vertices, edges = LoadSurface(offname,diroff)
points = LoadCenterLine(centerlinename,dirtxt)
